...
// @Router       /api/user/rename [post]
func RenameHandler(c *gin.Context) {
	var req RenameRequest
	if err := c.ShouldBindJSON(&req); err != nil || req.UserID == "" || strings.TrimSpace(req.Name) == "" {
	...
	if req.GameID != "" {
		logAndSend(req.GameID, req.UserID, "status", old+": Player renamed to "+user.Name)
